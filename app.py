...
def desenhar_mapa_pinos(df_pontos, gdf_mapa, cy, cx, zoom, pinos_bases=None):
    ...
    for _, row in df_pontos.iterrows():
        ...
        if bairros_selecionados and row['Bairro'] not in bairros_selecionados: continue
    ...
    if pinos_bases:
        for base, coords in pinos_bases.items():
            if base in transp_selecionadas_sidebar and base != TAG_MISSORTING:
...
# STREAMING_CHUNK:Aba 2 - Smart Routing com Limites em Cascata...
with aba2:
        ...
        if submit_ia:
            with st.spinner("Calculando matriz global de distâncias para todos os Cabeças de CEP..."):
                try:
                    ...
                    # A última base varre o que sobrou (Fechamento)
